...
const createDocumentation = (doc: { summary?: string; description?: string }) => {
 ...
 * @returns
 */
const createOperation = (method: MethodName, path: PathName) => {
  const sanitizedPath = path
    .replace(/^\//, "")
    .replace(/\{(\w)(.*)\}/g, (_, v1, v2) => `for${v1.toLocaleUpperCase()}${v2}`)
